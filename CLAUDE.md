...
6. **Photos and depth on published cities.** The 8-of-10 photo target and pin work continue as the background lane, no longer gating anything (the 13 documented dead-end gaps from the depth phase wait for UGC or new sources; do not re-run exhausted hunts). Hunt photos for NEW cities during their research, hard, per Step 4; two honesty rules never bend: no photo without a verified open licence and attribution, and a genuinely unreachable target is recorded in CURATION.md, never looped on.
   ...
   **One photograph per city is a standing aim, and it ranks BELOW five trees (Hidde, 2026-08-17: "can we make part of the nightly runs that we aim to get at least 1 photo per city", then "focus should be first get it to +5 trees than get photos").** 45 of 127 published cities have no photograph at all. The order he set is not a detail: a city under five trees has a TREE problem, and a photograph on a three-tree page does not make it an afternoon out. So a city joins the photo aim only once it clears five, and `scripts/photo_gaps.py` enforces that split rather than leaving it to memory (`--shortlist` prints one candidate per photo-less city, biggest city first).

   **The work splits across the two kinds of run, and the split is physical rather than a preference.** A NIGHT RUN cannot judge a photograph: the CI runner's egress proxy blocks upload.wikimedia.org, which cost a whole window to discover on 2026-08-07, and approving an image without looking at it is forbidden anyway. So a night run does the free half, which is real work: `photo_hunt.py --recheck`, `famous_trees.py --country <name>`, and leaving `photo_gaps.py --shortlist` behind. A SESSION does the viewing pass, 20 to 30 images in one context, against the Cadiz standard.
...
**The signup table is not optional and gets its own table (Hidde, 2026-08-16: "waarom heb je trouwens niks over de app inschrijvingen gezegd in de daily digest die moet daar in onthou dat en account registraties").** It carries **app waitlist signups, account registrations and reader submissions**, each with yesterday's number, the 14-day number and the running total. A digest that folds them into one line of a funnel table has technically reported them and has actually hidden them, which is what happened on 2026-08-16.
...
**Rule one: supply comes from the cheap end, in this order.** (a) Write what is already verified: `python3 scripts/leads.py` shows roughly 300 READY leads needing only a story, and a write pass is the one assembly-line stage measured under the 15k target, ~12k per tree, with the verify work in front of it already paid for. Batch 15 to 20 trees across cities per BRIEF_WRITING.md. (b) Then register-backed verify, dispatched only where the trees already have a register entry, a Wikidata candidate or equivalent hard data, so the pass only confirms alive and publicly reachable: that is why Lisbon and Porto cost almost nothing on 2026-08-12, and register work measures ~0.4k per tree against ~46k for verify without one. (c) Then import a new register, because scouting compounds and research does not; Den Bosch is the Dutch pattern, a municipal ArcGIS endpoint carrying owner and vitality, which answers hard rule 10 from the data itself. (d) From-zero web research on a city is off unless Hidde asks for that city by name: Baarn cost 556k tokens across three passes for zero trees.
...
10. **Publish public trees only, and never a location its source deliberately withholds.** Decided by Hidde on 2026-07-21. Some registers hide or blur the position of particular trees, and the Woodland Trust does this openly, because visitors damage ancient trees: soil compaction over the root plate, climbing, carving, and simple footfall. Others stand on private land whose owner never invited anyone. This site publishes exact coordinates and actively sends people to the trunk, which makes it capable of causing that harm rather than merely reporting it.
    ...
    **The access half is narrower than it was read, loosened by Hidde on 2026-08-13.** His question: "hoe erg is het nou als een boom op een schoolplein of kerk stad of zelfs bedrijfsterrein, vaak kun je die gewoon gaan bekijken toch." He is right, and the text always allowed it ("unless it is genuinely open to visitors"); what blocked those trees was the sentence that used to follow, "when in doubt, leave it out", applied to a doubt it was never written for. The rule was written about HARM, a tree damaged by visitors and a person's home invaded. A gate that might be shut is not harm, it is INCONVENIENCE, and the honest answer to inconvenience on this site has always been to say so rather than to hide the thing, exactly as `location_precision` does for a rough pin. So the deleted sentence is replaced by three tests, and a tree that passes all three ships:
    ...
    - **Can they get in without asking anyone's permission?** A churchyard, a cathedral close, a hospital garden, a campus, a company's grounds where the gate stands open: yes. A reception desk to talk past, a doorbell, a code, an appointment, a guard who checks: no. Paid entry is a yes, not a no, as long as `access` says so.
