...
**Ten is no longer the default. `CITY_QUEUE.md`'s `target` column is, and it is computed from supply rather than chosen** (`python3 scripts/city_queue.py`; the bands and their reasoning live in that file, which is the only place they live). His shape for it: easy and good supply means a big city may run to about 50 like Barcelona, hard to find means leave it, 20 to 30 is a fine first version, past 50 is a session decision with him. The measurement that makes it a rule rather than a wish: 68 of 95 live cities have almost no register supply within reach and 27 do, so for two thirds of the map the count is decided by what exists, not by ambition. A target is a ceiling on ambition, never a quota to fill and never a floor: a city that runs out of trees clearing the bar is finished below its target, and a city above it (Rome at 24 against 20) is finished, with nothing ever removed to meet a number.
