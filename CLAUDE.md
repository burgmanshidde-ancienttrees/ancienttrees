...
**Ten is no longer the default. A lot of cities at 10 to 25 trees is, and deepening comes later** (Hidde, 2026-08-12: "ik denk dat we veel beter een hoop steden met 10-25 bomen en dan later verdiepen"). `CITY_QUEUE.md`'s `target` column carries it, regenerated by `python3 scripts/city_queue.py`, and that file is the only place the rule and its reasoning live. One ceiling of 25 for every city with anything cheap within reach; no target at all for the 68 of 95 live cities that have almost nothing, which simply get whatever verifies. Past 25 is a deepening decision on a city that has shown demand, taken with him, never a run's target. A target is a ceiling on ambition, never a quota to fill and never a floor: a city that runs out of trees clearing the bar is finished below its target, and a city above it (Barcelona at 46) is finished, with nothing ever removed to meet a number.
