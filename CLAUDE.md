...
6. **New cities: frozen until the depth exit test passes, then the Netherlands wave.** The exit test: the eight cities search already serves (Amsterdam, Lisbon, Istanbul, Rome, Tokyo, Paris, London, Barcelona) each at 8 or more photos and 2 or fewer approximate pins, Amsterdam at zero. Once that is true a run may reopen coverage by itself, in this order and with counts per the doctrine in Step 1: first the Netherlands dense (Utrecht, Rotterdam, The Hague, Haarlem, Groningen, Leiden, Delft, Maastricht, Nijmegen, Breda, Den Bosch, Arnhem), then the UK (York, Bath, Oxford, Cambridge, Bristol, Glasgow), then Ireland (Cork, Galway). Pins from municipal government open data where available, licence read first; the national NL register is CC-BY-NC and stays a lead list.
