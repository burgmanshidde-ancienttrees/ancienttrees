...
6. **New cities: frozen until the depth exit test passes.** The exit test: the eight cities search already serves (Amsterdam, Lisbon, Istanbul, Rome, Tokyo, Paris, London, Barcelona) each at 8 or more photos and 2 or fewer approximate pins, Amsterdam at zero. Once that is true a run may reopen coverage by itself, in this order and with counts per the doctrine in Step 1:
   - **First the Japan wave (Hidde, 2026-07-28), built around his own trip.** Trip-first core, in his words: Nara, Kyoto and Tokyo deepened, Kanazawa, Fukuoka, Miyazaki, Kagoshima; wave tail afterwards: Kamakura, Nikko, Osaka. The four-tree bar stands per city: a trip city that cannot field four verified trees gets its best tree in a collection instead of a thin page. Kyushu leads to verify, not assume (the Kamou Great Camphor near Kagoshima, Fukuoka's shrine camphors); anchor data source: the national and prefectural Natural Monument tree registers; grounds and demand scan in GO_TO_MARKET.md "The Japan wave". **Hard deadline: his trip starts 2026-09-05; the wave, routes and photos included, must be complete by 2026-08-22.** When choosing between a new tail city and finishing a trip city properly, the trip city always wins.
   - Then the UK dense (York, Bath, Oxford, Cambridge, Bristol, Glasgow, and further UK cities that clear the four-tree bar), then Ireland (Cork, Galway), then the remaining European traveler capitals (Budapest, Krakow, Copenhagen, Stockholm, Dubrovnik, Zurich and onward; travelers search in English everywhere), then the Netherlands (Utrecht, Rotterdam, The Hague and onward: locals search in Dutch, so NL waits for Dutch pages or local seeding, and returns to the front the moment either exists; Amsterdam stays the reference city regardless). NL pins from municipal open data where available, licence read first; the national NL register is CC-BY-NC and stays a lead list.
   - **How this order gets to change (Hidde, 2026-07-28): the rollout is his strategy, not a formula's output and not a metric's.** Site data is far too small to steer on, and market scans inform rather than decide. A run may never reorder the waves from its own reasoning; a pre-wave demand scan stays as a cheap pre-flight check whose findings go to Hidde, and only he moves the queue. (The history of this ordering, including the NL-first reversal, lives in DECISIONS.md.)
