...
4. **New coverage: the Japan wave, then the recorded wave order.** Reopened by Hidde 2026-07-30 ("die hele kwaliteitsstop kan eraf, we gaan voor kwantiteit"); the depth exit test is retired, its history in DECISIONS.md.
   ...
   - **Current focus, ruled by Hidde 2026-08-06: depth on the demand cities, Rome, Lisbon and Porto first.** His instinct, confirmed against the data in session: breadth in the big register cities is nearly finished (46 of 56 major cities live), the marginal broad city is a Cernobbio nobody searches for, and the only pages with real demand signals are Rome (77 impressions, 0 clicks, position 17 for "trees in rome"), Lisbon (the only converting page) and Porto (a finished 17-tree walk with 4 photos). London was considered and dropped: no register, no Commons photos, no search signal, the most expensive city to deepen with no evidence anyone arrives. **"Deep enough" has a definition, so depth cannot become the new padding: a city is deep enough when it has at least one complete walk of 4 to 8 trees inside roughly 2 km, photos on most of that walk, honest pins, and season data.** Past that bar the marginal tree adds nearly nothing; Porto is already deep enough on trees and needs only photos, and Barcelona's 32 was a register accident, not a target. Breadth continues in the background at register cost only. This supersedes the 2026-08-02 Spain-and-Japan focus (history in DECISIONS.md).
...
**The ratchet, and it is the point of the whole loop: a lesson that appears on two different days stops being a note and becomes a build check.** Five already exist by that route: copy promising a tree count the city does not have, one species under two common names, a register file with no licence block, a phenology curve that records real moments and still draws flat, and a city published under its local name rather than the English one. When a walk or a retro finds a class of error twice, the fix ships with the check that makes it unshippable, and removing one of those checks needs Hidde.
