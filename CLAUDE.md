...
3. **Model tiering.** Verification passes dispatch on Sonnet (retrieval-heavy, the middle tier; never the smallest model, because verification is where the fabrication risk lives). Writing passes and anything touching judgement about what ships stay on the strong model. Anything deterministic (clustering, register parsing, photo candidate retrieval) is a script and costs nothing.
