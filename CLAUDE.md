...
**Ten is no longer a default, it is the first stage. Ten everywhere, twenty-five once Google confirms** (Hidde, 2026-08-12: "is het niet beter om overal eerst minstens 10 bomen te doen en dan te verdiepen wanneer we google search confirmatie krijgen"). `CITY_QUEUE.md` holds the rule and `data/city-queue.json` the number per city, regenerated by `python3 scripts/city_queue.py`; `--next` prints where to work and says to stop at the target. Every city aims at 10, which is a real page with a real walk. Only cities where Search Console has actually spoken carry 25, which is the same rule that governs photos and pins: depth follows demand rather than preceding it. Less is fine and expected where the trees are genuinely hard to find, so a city that runs out of trees clearing the bar is finished below its target and Cadiz at 5 is not a failure. A target is a ceiling and a stopping point, never a quota and never a floor: a city above it (Barcelona at 46) is finished, with nothing ever removed to meet a number.
