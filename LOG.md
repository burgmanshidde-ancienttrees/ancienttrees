...
Since a real broken link existed, ran a site-wide check comparing every stored photo URL's hash path against Commons' current API-reported path. Got through about 100 of 177 photo URLs (Amsterdam through roughly Palermo, alphabetically) before hitting the same rate limit again. Zero further mismatches found in what was checked; the remaining ~75 cities are unchecked, not confirmed clean, and worth finishing in a future pass. Full detail in CURATION.md.
