...
Since a real broken link existed, ran a site-wide check comparing every stored photo URL's hash path against Commons' current API-reported path. Got through about 100 of 177 photo URLs (Amsterdam through roughly Palermo, alphabetically) before hitting the same rate limit again, and it found two more genuine dead links: New York's Camperdown Elm and Queens Giant, both fixed the same way (correct path found, image fetched and viewed to confirm it's still the right tree, then the URL swapped). Three real broken links found and fixed this pass, zero false positives. A slower follow-up pass is checking the remaining ~75 cities (Nice through Vienna). Full detail in CURATION.md.
