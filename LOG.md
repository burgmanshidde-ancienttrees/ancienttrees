...
## 2026-07-30 — Same session, continued: the register layer's first data file ships (Tokyo, 28 trees from the metropolitan government's own CC-BY register), the four unscouted Kyushu/Hokuriku prefectures closed out as dead ends
...
**Built `data/registers/tokyo.json`, 28 trees, the register layer's first data file.** Tokyo's own CC-BY-4.0 cultural-properties CSV (verified via the catalog's own API, since the dataset's HTML page turns out to now sit behind an AWS WAF bot-challenge that blocks a plain fetch, a genuine change from whenever this source was last checked) gives 35 rows tagged Natural Monument. Filtered to 28: excluded 3 that are geological, not trees (a limestone cave, a limestone area, a rock dike), and 4 more per hard rule 10 because the register's own owner field names a private individual and nothing here independently confirms public access, left out rather than guessed at. Every entry `location_precision: approximate` (register coordinates are institution-level, e.g. two trees at the same temple share one point). Not wired into any page or the map yet, on purpose: the file itself is the smallest complete step in the build order, and it changes nothing about what ships publicly. No pages generated from it, nothing collectible, matching the pilot's own "look-only" rule.

One data file added (`data/registers/tokyo.json`), OPEN_DATA_SURVEY.md updated with the scouting results and the import's own accounting. Build and QA clean (this file isn't read by build_site.py yet, so nothing on the live site changed). Nothing needs Hidde; the explore-map layer (wiring these dots onto `/explore`) is the next build-order step for whichever run or session picks this up.
