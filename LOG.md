...
**FOR HIDDE, nothing blocking.** One thing worth knowing: if the map genuinely renders blank in CI's simulator (no GPU, or a tile host the runner's egress proxy blocks, the same shape as the documented Wikimedia/Openverse blocks), that would explain both `testTappingAPinOpensItsTree`'s original "no tree at all" failures on other runs and possibly the appfit hang, and it is worth a session with real Xcode access checking whether MapLibre draws anything at all on the CI simulator rather than assuming today's OAuth fix was the whole story.
