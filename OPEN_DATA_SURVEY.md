...
## California: a real state champion-tree API exists behind a React SPA, licence unstated (found on Long Beach, 2026-09-04)

**A register that would reach most of the ranked California cities at once, found by reading a webapp's own JS bundle rather than any open-data portal.** `scout_next.py --target` named Long Beach (#60) as no supply, no verdict. Cal Poly's Urban Forest Ecosystems Institute runs `californiabigtrees.calpoly.edu`, a client-rendered app with nothing in its server HTML; its bundle calls `https://www.selectree.calpoly.edu/api/bigtrees/getAllBt`, an undocumented but public JSON endpoint, no key required, returning all **266 current California state and national champion trees**: species (scientific and common), county, latitude/longitude, height, crown width, circumference, date measured, and `state_champion`/`national_champion`/`co_champion`/`hidden`/`dead` flags. This is a semantic filter, not a bulk inventory: the record is the biggest known specimen of its species in the state, which is exactly the "genuinely remarkable" bar this project already applies.

**The find is bigger than one city.** 55 of the 266 sit in Los Angeles County, 24 in Santa Barbara, 15 in Sacramento, 13 in Orange, 12 in Santa Clara, 9 in San Diego, 5 in Alameda, spread across 48 counties total. 13 sit within 30 km of Long Beach alone, mostly ornamental national champions (a Ficus triangularis 3.1 km out, Ficus microcarpa, Melia azedarach, Schinus terebinthifolia, Canary Island pine), the flavour of tree that reads well on a city page. If the licence clears, this is a California-wide unlock on the order of Flanders or Portugal, not a one-city fix.

**Not imported: no licence anywhere.** Neither `californiabigtrees.calpoly.edu` nor its parent `ufei.calpoly.edu` states a copyright notice, licence or reuse terms; the only disclaimer found on either site is an accuracy waiver ("No warranties or guarantees as to the accuracy of the data..."), which speaks to correctness, not reuse rights. Per the register-layer rule, an unstated licence is not a verified open one. Treated the way Toronto's empty-`licenseInfo` ArcGIS layer was treated: held rather than scraped, pending a licence ask (`data/register-scouting.json`, place "Long Beach"). It also carries no public/private access field, unlike Toronto's, so a licence grant would still need a per-tree access check under hard rule 10 before anything ships.
...
The Florida Forest Service publishes the register as a live ArcGIS Feature Service, item `cdddc319dfba422998d9e701c74d2d03`, layer 0 at `https://services3.arcgis.com/XYg2eF8UuxZVuVmF/arcgis/rest/services/Florida_Champion_Trees_Public_View/FeatureServer/0`. 404 rows statewide: common name, scientific name, status (Florida Champion / Florida Challenger / co-champion / pending), county, ownership, lat/long, location text, circumference, height, crown max/min, date measured, total points. **Units are inches and feet, not cm/m**, confirmed by reconstructing the American Forests scoring formula (`circumference_in + height_ft + avg_crown_ft/4`) against the register's own `TOTAL_PTS` field across all 43 Broward County rows: exact match every time. Convert before using anything from this register; the columns give no unit hint themselves and a cm/inch mix-up here would be the same error CLAUDE.md already recorded for ICNF's metres-not-centimetres girth column.

**Licence: the ArcGIS item's `licenseInfo` is a bare accuracy disclaimer** ("No warranties are provided for data therein, its use, or its interpretation"), not an open licence grant. Not clean enough for register-layer bulk dot import. Ruled the same as Nevada's register: **usable as a government verification source for hand-curated layer-1 work** ("ik vertrouw de overheid wel" applies to trusting a single government fact, not to republishing the dataset wholesale), each entry still needing its own second source and its own written story.

**One dense cluster shipped this pass: Flamingo Gardens, Davie (Broward County), opened as Fort Lauderdale (#57, day-trip distance, ~18km/20-25 min drive, no public transit).** 25 of Broward's 43 register rows sit at this one private botanical garden, more Florida state-champion trees in one place than anywhere else found so far; Wikipedia's own Flamingo Gardens article independently names several of the same species as the garden's champions (Enterolobium cyclocarpum, Hura crepitans, Tabebuia rosea, Brosimum alicastrum among them), which is what made a real second source possible without visiting. Four published (ftl_001-004). The other Broward rows are scattered free public sites (Hollywood's Young Circle baobabs, Pompano Beach, Lauderdale-by-the-Sea, Plantation Woods Park) that could balance the page's paid-entry ratio later; the Young Circle baobab's register circumference (416in = 10.6m) could not be corroborated by a second independent measurement this pass (monumentaltrees.com's matching entry is Cloudflare-gated, no Wayback snapshot exists) and was deliberately left unpublished rather than shipped on an unverified outlier figure.
...
- **Navarra**, 47 named trees, per-tree coordinates, and a per-tree official PDF fact sheet, which is a second-source generator in itself. Shapefile at `https://idena.navarra.es/descargas/BIODIV_Sym_MonumNat.zip`. The ISO metadata inside the download states CC BY 4.0 and the required wording: "Información geográfica propiedad del Gobierno de Navarra." A companion layer covers 8 trees in the Pamplona river park, a walkable cluster on its own, **imported 2026-09-16** as `data/registers/navarra-parflu.json` (`scripts/import_navarra_parflu.py`), licence independently confirmed on the dataset's own datosabiertos.navarra.es catalogue page (CC BY 4.0, no non-commercial clause). No individual names and no age/declaration year, just free-text Spanish descriptions mixing diameter figures with condition notes; a verify pass has to read and convert those, not trust a bare number. This is what Pamplona (#6 in CITY_QUEUE.md, previously zero supply) opens from.
...
## São Paulo, Brazil (#60): the decree that names trees exists, the dataset does not (scouted 2026-09-23)

Found while `scout_next.py --target` named Sao Paulo (#60) as no supply, no verdict, and no prior mention of Brazil anywhere in this file or `data/register-scouting.json`. The country does have the right legal concept: many Brazilian municipalities designate individual trees **imunes ao corte** (immune from cutting), which is exactly the semantic-filter shape this project wants. What is missing at the city end is a machine-readable dataset behind it.

**The instrument is real but it is a decree, not data.** State Decree no. 30.443 of 20 September 1989 declares certain trees in the municipality of Sao Paulo environmental heritage, immune to cutting. Read directly (`al.sp.gov.br`), it protects almost entirely by PLACE rather than by specimen: whole named parks and reserves (Parque Ibirapuera, Reserva Estadual da Cantareira) get blanket protection for every tree inside them, named plazas (Praca da Republica, Vale do Anhangabau) and institutional grounds (Palacio dos Bandeirantes) likewise, and residential/industrial lots are named by street address. Only Article 16 names **individual isolated specimens** by species and address (e.g. a Paineira at Av. Cristo Rei 101), and that list is short and embedded in decree prose. No coordinates anywhere, no structured per-tree table beyond that handful, no age or girth field. This is the same shape CLAUDE.md already documents for Sweden's naturminne decisions and Bavaria's Sicherungsverordnung: the protecting instrument is a legal DOCUMENT, and here, unlike those two, nobody has since mirrored it into a queryable list (no Wikipedia "Lista de arvores notaveis de Sao Paulo" exists; checked and not found).

**GeoSampa, the specific lead in the brief, does map the decree, but as landcover, not as trees.** The municipal GIS portal (geosampa.prefeitura.sp.gov.br) carries a layer under "Arborizacao Urbana" called "Vegetacao Significativa" (most recently the 2023 revision), which visualises the areas the 1989 decree and later municipal green plans protect. Confirmed via its own GeoNetwork metadata record (`metadados.geosampa.prefeitura.sp.gov.br`, layer "Cartas Vegetacao 1988 - Vegetacao Significativa"): the geometry type is **Surface** (polygons), not points, covering 44.42 percent of the municipality's territory. It is a bulk vegetation-cover layer, not a semantic filter to individually designated remarkable specimens, and CLAUDE.md's register-layer rule excludes exactly this shape ("never full municipal tree inventories"). GeoSampa separately publishes a general street-tree inventory of roughly 650,000 trees, which is the same bulk-inventory problem at a larger scale. A third GeoSampa layer, heritage-listed assets ("bens tombados", live since 2017), covers buildings and heritage urban zones per the municipality's own announcement, with no mention of individual trees.

**Licence would be a second blocker even if a tree-point layer existed.** The Vegetacao Significativa metadata record states access "sem restricoes" but use "direitos de propriedade intelectual aplicam-se" (intellectual property rights apply), states no formal licence, and directs reuse questions to geosampa@prefeitura.sp.gov.br. This is inconsistent with the generic prefeitura.sp.gov.br site footer, which claims all site content is under a Creative Commons licence without naming the variant. Per the register-layer rule (and the lesson recorded here about the Dutch LRMB, where an unread licence closed a door for months), that inconsistency is exactly the kind of thing that needs a written proving sentence before import, not an inference either way.

**The city's own open-data portal has nothing of this kind either.** `dados.prefeitura.sp.gov.br` (CKAN, roughly 401 datasets) returns, for "arvore"/"arvores", only operational logs: pruning reports, authorised-removal reports and planting spreadsheets by subprefeitura ("Relatorio de Manejo de Arvores", "Podas Emergenciais"), which are management records rather than a semantically filtered heritage list.
