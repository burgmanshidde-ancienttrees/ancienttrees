...
12. **A feature whose value accrues over time must be reasoned about over time.** The origin story, moved verbatim from CLAUDE.md 2026-07-29 (the rule itself lives there, "Walk the user's timeline before you build"): On 2026-07-21 a run shipped the tree passport, verified it thoroughly, and shipped a feature that would have silently deleted people's collections: browser storage on iOS Safari is wiped after seven days without a visit, which is exactly the three week trip the feature exists for. Everything tested passed. The tests were about whether it worked, over thirty seconds, when the entire value of collecting is measured in months. Hidde spotted it immediately, and he was right that it was obvious. The lesson is not "know more about Safari". It is that knowing something does not help if nothing prompts you to ask.

13. **No gates before demand.** The origin story, moved verbatim from CLAUDE.md 2026-07-29 (the rule lives there, "Ship rough, ship wide, fix later"): On 2026-07-21 a run built a quality floor that blocked new cities until existing ones were polished, at five cities, with no users. It was well argued and completely wrong: it optimised for a standard nobody was measuring yet, and it would have cost three months of coverage. The shape to recognise is a mechanism that enforces polish before there is demand for it. If you catch yourself designing a gate, a threshold or a floor, ask what evidence says anyone is waiting on the other side of it. Usually the honest answer is nobody, and the gate should not exist.

14. **One reader's problem with one tree is a fix for that tree, not a component on every page (Hidde, 2026-08-13).** A journalist walked to the Bastion Oranje maple, could not tell which of several similar trunks was ours, and said so. That is the best kind of feedback and it was worth acting on. What shipped instead was a schema field, a build check and a boxed "Which one is it?" block rendered above the story on every tree card of every city page, ready to appear on 1,083 trees. His words on seeing it: "compleet belachelijk om op 1 reporter overal opeens neer te gaan zetten... kan je op de dieperliggende pagina van die boom wat tips geven." The block came off the city page the same hour; it stays on the tree's own page, where somebody standing in front of the tree is actually looking.

    The shape to recognise, because it is flattering and therefore easy to miss: a single vivid complaint arrives, it feels like a discovered law of the product, and the fix is built at the level of the system rather than at the level of the thing that was wrong. The test is one question. How many readers reported this, and does the fix live at that scale? One tree gets a sentence on one page. A pattern across many trees earns a component, and the evidence for that comes later or not at all. Note this is not the reverse of "the fix ships with a check that makes it unshippable": that ratchet is about OUR recurring errors, which we can count. This is about a reader's single report, which we cannot.
