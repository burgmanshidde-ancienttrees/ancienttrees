...
| Publish gate | **5+ mapped trees in the park AND a hand-written intro in `data/parks/[slug].json`.** Both, or no page. Waivable per park by Hidde only, down to 3, via a `below_gate` block in that park's own intro file carrying who approved it and in whose words (v1.19). |
