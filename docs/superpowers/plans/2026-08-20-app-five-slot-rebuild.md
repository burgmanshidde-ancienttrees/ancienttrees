...
- [ ] **Step 3:** Composition sweep per screen against the teardown: cards, headers, stat rows, one primary. **The named bugs from Hidde's confirmed findings list, each checked off individually, not as "sweep":** (a) no literal tab-label headings ("Home"/"Collect"/"Profile" as page titles die; content is the heading); (b) content must not scroll visibly behind the floating tab bar on Explore (bottom safe-area inset on the scroll view); (c) the tree page title renders once, not in both the nav bar and the body; (d) "Take me there" must not overlap the map inset; (e) the tab bar hides on pushed tree/walk pages (`.toolbar(.hidden, for: .tabBar)` on detail destinations, the AllTrails behaviour); (f) one search field in the app's chrome, owned by Explore.
- [ ] **Step 3b: The editorial hero rule, wired not hoped:** every shelf and sheet leads with the best photo-carrying tree (approved photo first, in-leaf over bare where knowable, distance as tiebreaker), never the raw nearest row; the map sheet's selected-tree card shows the species mark with "No photograph of this tree yet" only when the tree genuinely has none, and the HERO slot on Explore never goes to a photo-less tree. Implemented as one shared `editorialRank(_:origin:)` used by Home shelves, the map sheet and the suggestion pickers from Tasks 3/5.
...
- Not in scope, said out loud: walk/place hearts in Saved (schema change), Spot photo capture (needs Hidde), Season Radar notifications (Plus, phase 2), any Kit semantics, and the walk **Begin mode** (route drawn full-screen, progress on the line, tick as you pass) — that is the next plan once this structure stands, per Hidde's confirmed findings list point 6.
