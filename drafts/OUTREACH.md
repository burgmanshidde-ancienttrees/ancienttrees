...
| 2026-09-08 | National Biodiversity Data Centre (info@biodiversityireland.ie) | Their CC BY Heritage Trees dataset gave us eight of Cork's thirteen and six of Dublin's seventeen. Asks whether the survey is still growing and whether there is a newer version to pull from. | Sent, batch-010. |
| 2026-09-04 | getLISBON (contact@getlisbon.com) | Reply to their own mail offering a mutual link and two jacarandas. Both trees published first (lis_034, lis_035), getLISBON cited with a link on each. Asks which two jacarandas stand in the Ajuda garden, a photograph if they have one, and that their PT article link to /pt/lisbon and the EN one to /lisbon. | **LIVE, checked 2026-09-08.** They placed both links exactly as asked: the EN article links to /lisbon, the PT one to /pt/lisbon, both `rel="noreferrer noopener"` with no nofollow, so it is a followed link. The pages are getlisbon.com/discovering/public-interest-tree-in-lisbon/ and /pt/descobrindo-pt/arvores-de-interesse-publico-em-lisboa/. First real backlink the project has, after 220 mails. Note for every future digest: `noreferrer` strips the referrer header, so clicks from it land in Cloudflare as `(direct)` and "external referrers: none yet" is NOT evidence the link is absent. Still open on their side: which two jacarandas stand in the Ajuda garden. **2026-09-06 she answered** (unanswered for five days until 2026-09-11): links placed, not sure which two jacarandas and will come back, no tree missing from our list, and she SENT a June photograph of the Tipu of São Bento (lis_005, which had none) as an attachment, 20260629_103909.jpg, 7.9 MB. A check of mine first reported it absent, because it only read the lines under the sender; Hidde caught it. Looked at, approved and published 2026-09-11 (28671050), credit getLISBON. Thank-you in `drafts/reply-getlisbon-2.md` **sent 2026-09-11** on his word, after the photo was verified live on /lisbon and /pt/lisbon. |
...
**Batch 010-app-launch, 2026-09-03, approved by Hidde in session on his one-word "verstuur".** Prompted by his own read that this is a legitimate reason to write to everyone already mailed: "ik denk dat je iedereen een mail weer kan sturen dat de ios app uit is". 212 recipients, all 210 non-manual addresses ever mailed minus the do-not-contact list, Paulo (dias.com.arvores@sapo.pt, active correspondence, no form re-announcement needed) and the Oslo contact who replied the same day (postmottak@bym.oslo.kommune.no), plus 4 addresses that turned out to be Hidde's own contributor-form test accounts, excluded as not real outreach targets. 10 people who actually corresponded (Hanna Hirsch, Georges Feterman, Jon Pattee, Giulia Torta, Raffy Hull, Paul O'Sullivan, Karen Sweeney, Vera Wesinger, Ales Rudl, Park Guell) got a short personal update instead of the template, each referencing their own last exchange rather than repeating an ask already answered. Everyone else got one of nine language templates (en/nl/pt/de/es/fr/it/pl/cz, plus ca for two Catalan addresses), built from the actual URL and city stored per-address in the original batch files, with city names localised for natural prose (Rome to Roma, Munich to Munchen, etc). Every mail carries a resend_reason since all 212 were mailed before. Daily cap of 50 sent 46 today (4 already used by an unrelated contributor reply); 166 remain queued in drafts/batches/batch-010-app-launch.json (status approved_by_hidde) for the send script to pick up on the following days. The point per CLAUDE.md's distribution section: zero of the ~240 prior outreach mails have produced a measurable backlink, so this is a second, genuine touch rather than a repeat of the same ask; it does not repeat the link request already made to Paul or Karen since neither wanted it repeated.
...
| 2026-09-17 | Quercus Lisboa came back through Hugo Verissimo (Direcao, arborist), who is organising a trial group walk with the app, will pick the zone and bring a biologist, will photograph the trees for us and offers promotion on their site and socials | Reply (drafts/reply-quercus-lisboa-2.md): the 11-tree 4.5 km Campo Santana to Jardim da Parada line so he has a route, the 22 Lisbon trees without a photograph, yes to the promotion, and one ask, an online meeting before the walk | **draft, awaiting Hidde** |
