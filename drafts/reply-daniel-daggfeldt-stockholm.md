# FOR HIDDE: reply to Daniel Daggfeldt, arborist in Stockholm

To: daniel@tradmastarna.se, **reply to all**: Tina Axelsson is on his thread
because he put her there ("so you can have a direct contact if you like to
proceed"). A separate mail to somebody who was cc'd reads as going round the
person who introduced her, and it splits a thread that is currently one
conversation. Hence the single line addressed to her at the bottom.

**Not sent yet.** Drafted 2026-09-08, cut to the bone 2026-09-10 on Hidde's
steer: "just ask him for feedback, and don't dwell on the tree details". What
changed on the pages is one line and a link; the question is the mail. Nothing in data/outreach-sent.json for this
address.

- **Valkasken is live and verified**, two independent sources: Swedish
  Wikipedia and Lansstyrelsen Stockholm's 2016 survey of sarskilt skyddsvarda
  trad, which counts it among 42 ashes worth protecting. His own map pin and
  Wikipedia's land five metres apart, so the pin is `confirmed` rather than
  approximate. Flagged, because nobody anywhere records a girth or a height for
  it. **If he writes back, ask for the girth.** He is an arborist and he owns a
  tape, and a girth is the one field that would let the tree be dated at all.
  Deliberately not asked here: this mail already carries the Prague question
  and the ask to Tina.
- **Both his corrections are applied** and the story is back under 250 words.
  He says he found nothing else wrong on the page, which is the first time
  anybody has audited one of ours.
- **He has not been to Prague yet.** "A trip coming up in a couple of weeks",
  so he planned with it rather than walked with it. The question is worded to
  say so; asking how he experienced the city would read as not having listened.
  Prague is 21 trees, 24th deepest of 565 places, so what he finds missing
  there is worth more than a compliment.
- **Tina Axelsson is worth more than the tree.** She has photographed all of
  these and Stockholm has three photographs across six trees. Nothing ships
  from her until there is a licence we can record: either she names one, or she
  gives written permission for use on the site with her credit. The mail asks
  only for the photographs and names the credit, because a licence paragraph
  in a first mail is process-pitching; the record gets made in her answer.
  Her name DOES go under the photographs: a photographer who was written to
  and answered keeps their credit, which is the opposite case to a reader
  whose name we never publish.
- **The two attached photographs he already sent (Prins Eugen oak, Valkasken)
  are not usable yet** for the same reason. They arrived as examples of her
  work, not as a grant.
- **He is a VETcert veteran tree specialist**, which is the qualification for
  exactly the trees this site is about, and he came through the Swedish Tree
  Association. If Sweden ever gets a proper pass, he is the door.

---

Hi Daniel,

Thanks, this is the mail I hoped for. Valkasken is live and both your
corrections to the Prince Eugen oak are in:
https://ancienttrees.app/stockholm/valkasken

Good to hear you used it to plan Prague. What did you think of it, and was
anything missing?

We put the app on the App Store a week ago and I would like to hear what you
make of that as well:
https://apps.apple.com/nl/app/ancient-trees/id6806177833?l=en-GB

Tina, Daniel says you have photographed most of these. I would love to put a
few of them on the tree pages with your name under them.

Hidde
