...
def check_app_downloads_are_their_own_block():
    """App Store downloads is its OWN digest block, never appended to the app
    table's.

    The ratchet, on the second showing of one lesson. Hidde, 2026-09-08: "ik
    mis app downloads in deze lijst", after the number kept arriving as a
    sentence under the app table. It got a table, and the table was built by
    appending lines INSIDE app_section, which reads the same and is not: on
    2026-09-17 a PostHog query timed out, block() caught it, and Apple's
    numbers vanished with a service they do not come from. Neither table was
    printed and the workflow went green.

    They are separate fetches from separate companies answering separate
    questions, so they fail separately. This refuses the fold coming back."""
    root = Path(__file__).resolve().parent.parent
    try:
        src = (root / "scripts" / "daily_digest.py").read_text(encoding="utf-8")
    except OSError as e:
        return [f"app-downloads check could not read daily_digest.py: {e}"]
    out = []
    body = src.split("def app_section(")[-1].split("\ndef ")[0]
    if "app_store_downloads_lines()" in body:
        out.append(
            "daily_digest.py: app_section calls app_store_downloads_lines(). "
            "App Store downloads is its own block (Hidde, 2026-09-08); folded "
            "in, a PostHog failure takes Apple's numbers down with it.")
    if "block(app_store_section" not in src:
        out.append(
            "daily_digest.py: main() does not call block(app_store_section). "
            "Without it the digest prints no App Store downloads at all.")
    return out
...
PARTS = {
    ...
    "tree-flag": "the report flag",
    "dir-link": "the directions button",
    "td-open": "the Open in the app button",
    "td-inset": "the map in the photograph's corner",
