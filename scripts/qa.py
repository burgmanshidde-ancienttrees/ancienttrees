...
def check_one_city_order():
    path = Path(__file__).resolve().parent.parent / "data" / "city-list.json"
    if not path.exists():
        return []
    ...
    if not bad:
        return []
    return [f"data/city-list.json: carries ordering field(s) {', '.join(bad)}; "
            "the one order lives in CITY_QUEUE.md. Run python3 scripts/city_queue.py"]
